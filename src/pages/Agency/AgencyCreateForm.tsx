...
export default function AgencyCreateForm({ registerData, setStep }: Props) {
  ...
  const handleChange = (key: keyof AgencyToAdd, value: any) => {
    setCompanyData(prev => ({
      ...prev,
      [key]: value,
    }));
  ...
  const validateData = (data: AgencyToAdd) => {
  ...
  const handleRegister = async (agencyId: string) => {
    try {
  ...
  const handleCompanyCreate = async () => {
    ...
    try {
      setLoading(true);
      const agencyId = await createAgency(companyData);
  ...
  return (
    <div className="flex bg-white rounded-2xl flex-col justify-center px-6 py-12 lg:px-8 shadow-lg p-6 w-90 sm:w-[45rem] z-10 relative">
      ...
      <div className="flex justify-center">
        <h1 className="text-3xl">Podaci agencije</h1>
      </div>
      <div className="mt-10">
